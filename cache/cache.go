...
func (c *Cache) OnReleaseChange(event *sfs.ReleaseChangeEvent) {
	...
	if err := os.MkdirAll(c.path, os.ModePerm); err != nil {
		logger.Errorf("mkdir cache path %s failed, err: %s", c.path, err.Error())
		return
	}
	...
	for _, ci := range pl.ReleaseMeta.CIMetas {
		...
		if err != nil {
			logger.Errorf("check config item exists failed, err: %s, rid: %s", err.Error(), event.Rid)
			continue
		}
		if exists {
			continue
		...
		if err := downloader.GetDownloader().Download(ci.PbFileMeta(), ci.RepositoryPath, ci.ContentSpec.ByteSize,
			...
			logger.Errorf("download file failed, err: %s, rid: %s", err.Error(), event.Rid)
			return
		}
	}
}
...
func (c *Cache) GetFileContent(ci *sfs.ConfigItemMetaV1) (bool, []byte) {
	exists, err := c.checkFileCacheExists(ci)
	if err != nil {
		logger.Errorf("check config item %s cache exists failed, err: %s", ci.ContentSpec.Signature, err.Error())
		return false, nil
	}
	if !exists {
		return false, nil
	...
	if err != nil {
		logger.Errorf("read config item cache file %s failed, err: %s", filePath, err.Error())
		return false, nil
	}
	return true, bytes
}
...
func (c *Cache) CopyToFile(ci *sfs.ConfigItemMetaV1, filePath string) bool {
	exists, err := c.checkFileCacheExists(ci)
	if err != nil {
		logger.Warnf("check config item %s cache exists failed, err: %s", ci.ContentSpec.Signature, err.Error())
		return false
	}
	if !exists {
		return false
	...
	if err != nil {
		logger.Errorf("open config item cache file %s failed, err: %s", cacheFilePath, err.Error())
		return false
	}
	defer src.Close()
	dst, err := os.OpenFile(filePath, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, os.ModePerm)
	if err != nil {
		logger.Errorf("open destination file %s failed, err: %s", filePath, err.Error())
		return false
	}
	defer dst.Close()
	if _, err := io.Copy(dst, src); err != nil {
		logger.Errorf("copy config item cache file %s to destination file %s failed, err: %s",
