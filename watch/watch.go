...
func (w *Watcher) OnReleaseChange(event *sfs.ReleaseChangeEvent) {
	...
	for _, subscriber := range w.subscribers {
		if subscriber.Opts.App == pl.Instance.App &&
			...
			if err := subscriber.Callback(pl.ReleaseMeta.ReleaseID, configItemFiles,
				...
				logs.Errorf("execute watch callback for app %s failed, err: %s", subscriber.Opts.App, err.Error())
			}
...
func (w *Watcher) Subscribe(callback Callback, opts ...SubscribeOption) *Subscriber {
