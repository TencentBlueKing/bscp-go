	...
	for _, f := range files {
		...
		g.Go(func() error {
			...
			if !exists {
				...
				logger.Infof("file %s is already exists and has not been modified, skip download", filePath)
			}
			// 3. set file permission
			if err := util.SetFilePermission(filePath, file.FileMeta.ConfigItemSpec.Permission); err != nil {
				logger.Warnf("set file permission for %s failed, err: %s", filePath, err.Error())
			}
			return nil
		})
	}
...
func CheckFileExists(absPath string, ci *sfs.ConfigItemMetaV1) (bool, error) {
	...
	if sha != ci.ContentSpec.Signature {
		logger.Infof("configuration item's SHA256 is not match, local: %s, remote: %s, need to update",
