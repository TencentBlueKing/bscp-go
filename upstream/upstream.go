...
func (uc *upstreamClient) dial() error {
	...
	if uc.conn != nil {
		if err := uc.conn.Close(); err != nil {
			logger.Errorf("close the previous connection failed, err: %s", err.Error())
	...
	logger.Infof("dial upstream server %s success.", endpoint)
...
func (uc *upstreamClient) ReconnectUpstreamServer() error {
	if !uc.wait.TryBlock() {
		logger.Warnf("received reconnect to upstream server request, but another reconnect is processing, ignore this")
		return nil
	}
